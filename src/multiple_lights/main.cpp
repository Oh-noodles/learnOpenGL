...
int main() {
  ...
  Shader objectShader("src/multiple_lights/shader.vs", "src/multiple_lights/lighting_object_shader.fs");
  Shader lightSourceShader("src/multiple_lights/shader.vs", "src/multiple_lights/light_source_shader.fs");
  ...
  unsigned char *texData = stbi_load("resources/textures/container2.png", &texWidth, &texHeight, &nrChannels, 0);
  if (texData) {
  ...
  texData = stbi_load("resources/textures/container2_specular.png", &texWidth, &texHeight, &nrChannels, 0);
  if (texData) {
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, texWidth, texHeight, 0, GL_RGBA, GL_UNSIGNED_BYTE, texData);
    glGenerateMipmap(GL_TEXTURE_2D);
  } else {
...
void renderObjects(Shader shader, unsigned int VAO) {
  ...
  shader.setVec3("dirLight.ambient", lightColor * glm::vec3(0.2f));
  shader.setVec3("dirLight.diffuse", lightColor * glm::vec3(0.5f));
